...
补齐与 feishu-claude-bridge 的**完整功能对等**——前两轮是挑着搬的，这次做了穷尽的文件级与
符号级对比，逐项补全。复查结果：功能面缺失 0 项。

### 新增

- **断连自动重试**：此前只搬来了 `abortRetries` 的基础设施，却没有实际的重试逻辑。
  现补上 `runCodexOnce` + 带退避的 `runCodex` 包装（默认 2 次，3s/15s）。
  只重试「还没动过手」的早期失败——模型一旦开始输出就可能已经写过文件、发过消息，
  整段重放会造成双写。
- **模型的 CLI 版本要求自检**（`MODEL_MIN_CLI`）：版本不够时报错发生在**用户发消息那一刻**
  （`requires a newer version of Codex`），而不是启动时。现在启动即告警，并指明是哪个路径下的哪个版本。
- **`normalizeModel` / `normalizeEffort` 抽为可复用函数**：此前只有 `setRuntimeConfig` 内联校验，
  定时任务里写的 `model` 别名走不到这层。现在 `/model`、`set-model` 动作、任务级 `model`
  三条路径行为一致。
- `.env.example` 补齐 13 个 v2.x 开关；README 补「权限边界」与「三层记忆」两节。

### 变更

- **运行配置改为随 prompt 注入，废弃 `workspace/runtime.md`**：那个文件被写入却从无人读取，
  而定时任务与聊天是并发的两个进程、共享同一工作区，写文件必然互相覆盖——
  模型会读到别人的 chat_id，进而把排期发错会话。
- 记忆注入措辞对齐三层结构（画像层 + 事实层索引随调用注入，流水层按需检索）。

## [2.1.0] - 2026-09-05

### 新增

- **三层记忆架构**（对齐 feishu-claude-bridge）：
  - 画像层 `memory/USER.md`——用户的身份、偏好、沟通与判断风格，随每次调用注入
  - 事实层 `memory/<slug>.md` + `MEMORY.md` 索引——长期有效的事实与口径
  - 流水层 `memory/journal/YYYY-MM-DD.md`——当天的过程细节，不占固定上下文，由关键词召回按需提示

  此前只有单层（索引 + 事实文件），没有画像层，也没有地方放"还不确定但值得记下"的东西。
  Codex 没有 `@import`，所以画像层与索引由桥接显式拼装后注入；访客一律不注入。
- **主动记忆规则**：不用等用户说「记住」——纠正假设、做出决定、表达偏好、给出数字口径、
  有时限的承诺，当场落盘；配套「何时不写」清单与 supersede 就地改写规则（不留矛盾条目）。
- **`USER.md` 同样走模板机制**：仓库只存 `USER.md.template`，运行时文件启动时生成并被 gitignore
  排除——与 `MEMORY.md` 一致，避免机器人写入的内容随提交进入公开仓库。
